export class Abacus {
  ...
  private container: HTMLElement | null; // querySelector could return `null` if the element is not found
  ...
  private bindEventHandlers() {
    ...
    this.container!.querySelector('#lessBtn')?.addEventListener(
      'click',
      this.handleNumberButtonClick.bind(this, -1)
    );
    this.container!.querySelector('#moreBtn')?.addEventListener(
      'click',
      this.handleNumberButtonClick.bind(this, 1)
    );
  ...
  protected handleNumberButtonClick(changeValue: number) {
  ...
  protected handleNumberInputChange(evt: Event) {
    evt.preventDefault();
    const numberInput = evt.target as HTMLInputElement;
  ...
  protected handleBeadClick(clickedBead: HTMLButtonElement) {
    /* Check if the bead has the class 'counted' */
    const isCounted = function (bead: Element) {
      return bead.classList.contains('counted');
    };

    const recursivelyAdjustBeads = (bead: HTMLButtonElement) => {
      const nextSibling = bead.nextElementSibling as HTMLButtonElement;
      const previousSibling = bead.previousElementSibling as HTMLButtonElement;
  ...
  protected getMoveDistance() {
    let beadWidth;
    const RIGHT_MARGIN = 2;
    const BEADS_PER_ROW = 10;
    const bead = this.container!.querySelector<HTMLElement>('.bead');
  ...
  protected moveBead(bead: HTMLButtonElement) {
  ...
  protected updateAbacus() {
    let total = parseInt(this.numberInput.value);
    const rowsDescending: number[] = this.rows.sort((a: number, b: number) =>
      a < b ? 1 : -1
    );
    ...
    rowsDescending.forEach((rowGroup) => {
      ...
      const rowBeads = this.container!.querySelector(
        `.row${rowGroup}`
      )?.querySelectorAll<HTMLButtonElement>('button.bead');
  ...
  private updateNumberInput() {
    ...
    this.rows.forEach((rowNumber) => {
      const countedBeads = this.container!.querySelector(
        `.row${rowNumber}`
      )?.querySelectorAll<HTMLButtonElement>('button.counted');
